$(document).ready(function(){
	...
	$("#approve_local").addClass("active_menu")
	$("#message").html("Approve / Deny a Local Raster Submission")
	resetPage()
	...
	function resetPage(){
		...
		$("#meta_list").empty()
		$("#meta_input").empty()
		$("#meta_list").append('<option id="blank_meta_list_item" class="meta_list_item" value="-----">Select submitted local</option>')

		scanDir({ type: "scan", dir: "../../uploads/locals/pending" }, function(options) {
    ...
    $("#meta_list").on("change", function(){
    	...
    	var file
    	scanDir({ type: "scan", dir: "../../uploads/locals/pending/"+local }, function(files) {
    	...
    	$("#meta_input").append('<a href="../../uploads/locals/pending/' + local +'/'+ file + '">' + file + '</a>')

		$.ajax ({
	        url: "process.php",
	        data: { type: "meta" , local: local },
	        dataType: "json",
	        type: "post",
	        async: false,
	    ...
	    $("#submit").click(function(){
			...
			$.ajax ({
		        url: "process.php",
		        data: { type: "crop" , local: local, file: file, data: $("#input_form").serialize() },
		        dataType: "text",
		        type: "post",
		        async: false,
		        success: function(result) {
		        	...
		        	$("#message").html("Local has been approved and processed")
		        	$('html, body').animate({ scrollTop: 0 }, 0);
		        	resetPage()
		        	window.location = self.location	
		        }
		...
		$("#reject").click(function(){
			...
			$.ajax ({
		        url: "process.php",
		        data: { type: "reject" , local: local, file: file, reason: $("#reason").val() },
		        dataType: "text",
		        type: "post",
		        async: false,
		        success: function(result) {
		        	console.log(result)
		        	// window.location = self.location
		        	$("#message").html("Local has been rejected")
		        	$('html, body').animate({ scrollTop: 0 }, 0);
		        	resetPage()
		        	window.location = self.location			        	
		        }
